...
class GitRepository (object):
    ...
    def __init__(self, path, force=False):
        ...
        if not (force or os.path.isdir(self.gitdir)):
            raise Exception("Not a Git repositiory %s" % path)
...
def object_resolve(repo, name):
    ...
    as_branch = ref_resolve(repo, "refs/rags/" + name)
    if as_branch:
...
def gitconfig_read():
    ...
    configfiles = [os.path.expanduser(os.path.join(xdg_config_home, "git/config")), os.path.expanduser("~/.gitconfig")]
